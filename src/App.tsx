...
async function speakWithFallback(text, onEnd = () => {}) {
  ...
  const pick = voices.find(v => v.lang.startsWith('it') && /female|woman|donna/i.test(v.name))
...
async function speakAll(messaggio, fatti, onEnd = () => {}) {
  if (!messaggio) { onEnd(); return; }
  const fattiTesto = fatti.length > 0
    ? 'E sapevi che nel tuo giorno di compleanno sono successe cose straordinarie? '
...
const SPOKEN_Q = {
  name: 'Sono lo Specchio Magico degli Auguri. Dimmi, come ti chiami?',
  date: (name) => `Piacere di conoscerti, ${name}! Quando sei nato? Di' o scrivi il tuo compleanno.`,
};
...
const buildPrompt = (name, day, month, daysUntil) => {
  ...
  let timing = '';
  if (daysUntil === 0)               timing = 'OGGI e il compleanno!';
  ...
  return (
    ...
    >
      ...
      <p style={{
        ...
        {isName
          ? 'Sono lo Specchio Magico degli Auguri. Come ti chiami?'
          : `Piacere di conoscerti, ${name}! Quando sei nato?`}
      </p>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
        ...
        >
          {isName ? 'Avanti ✨' : 'Mostra il mio messaggio 🪄'}
        </button>
      </div>
...
// ── Nuvoletta del messaggio (italiano) ────────────────────────────────────
      ...
      {fatti.length > 0 && (
        <div style={{ borderTop:'1px solid rgba(212,160,23,0.16)', paddingTop:8,
          ...
          <p style={{ margin:0, fontSize:9, color:'rgba(212,160,23,0.46)',
            ...
            ✦ Nel tuo giorno di compleanno, in passato ✦
          </p>
          {fatti.map((f,i) => (
            <div key={i} style={{
...
export default function SpecchioAuguri() {
  ...
  // Pronuncia la domanda ad ogni passaggio
  useEffect(() => {
    let cancelled = false;
  ...
  const buildFallback = (n, day, month, daysUntil) => {
    ...
    let saluto = '';
    if (daysUntil === 0)
      saluto = `Oggi e il tuo grande giorno, ${n}! Tutto il mondo e felice che tu ci sia! 🎉`;
    else if (daysUntil > 0 && daysUntil <= 7)
      saluto = `Mancano solo ${daysUntil} giorno${daysUntil===1?'':'i'}, ${n}! Il tuo compleanno sta arrivando! 🎈`;
    else if (daysUntil < 0 && daysUntil >= -7)
      saluto = `Tanti auguri, ${n}! ${Math.abs(daysUntil)} giorno${Math.abs(daysUntil)===1?'':'i'} fa era il tuo giorno speciale. Spero tu lo stia ancora festeggiando! 🎂`;
    else
      saluto = `Che meraviglia essere nati il ${day} ${mese}, ${n}! E un giorno davvero magico! 🌟`;

    const messaggio = `${saluto} Lo Specchio Magico sa con certezza che tu sei una persona speciale, perche nel giorno del tuo compleanno c'e sempre un po' di magia nell'aria. Chiudi gli occhi ed esprimi un desiderio — a volte si avverano davvero! ✨`;

    const fattiFallback = {
      inverno: [
        { anno: 1812, it: 'I fratelli Grimm scrissero il loro primo libro di fiabe pieno di storie magiche per bambini.' },
        { anno: 1879, it: 'Per la prima volta fu usata la luce elettrica, come una bacchetta magica che illumina la notte!' },
        { anno: 1955, it: 'Apri le porte il primo Disneyland, un vero parco delle fiabe pieno di sogni.' },
      ],
      primavera: [
        { anno: 1902, it: 'Usci il primo libro sulla Terra di Oz, con il famoso Mago.' },
        { anno: 1937, it: 'Biancaneve fu il primo lungometraggio animato — e vissero tutti felici e contenti!' },
        { anno: 1989, it: 'La Sirenetta nuoto per la prima volta sullo schermo, un mondo magico sott\'acqua.' },
      ],
      estate: [
        { anno: 1865, it: 'Alice cadde per la prima volta nel buco del coniglio e arrivo nel Paese delle Meraviglie.' },
        { anno: 1977, it: 'Luke Skywalker volo per la prima volta tra le stelle — una fiaba nello spazio!' },
        { anno: 1997, it: 'Harry Potter sali per la prima volta sulla sua scopa e volo verso Hogwarts.' },
      ],
      autunno: [
        { anno: 1889, it: 'La Torre Eiffel apri le sue porte, alta come il cappello di un mago!' },
        { anno: 1928, it: 'Topolino squittì per la prima volta, l\'inizio di un mondo magico di cartoni animati.' },
        { anno: 1952, it: 'Pippi Calzelunghe apparve per la prima volta in televisione, la bambina piu forte del mondo.' },
      ],
    };
  ...
  const fetchMessage = async (n, day, month, days) => {
    ...
    try {
      ...
      if (data.it) {
        setIsSpeaking(true);
        speakAll(data.it, data.fatti || [], () => setIsSpeaking(false));
      }
    } catch (err) {
      const fallback = buildFallback(n, day, month, days);
      setMessage(fallback);
      setStatus('✨ Lo specchio parla con la sua voce magica...');
  ...
  const banner = (() => {
    ...
    if (daysInfo === 0)              return { text:'🎂 Oggi e il tuo grande giorno!', color:'#f5e642' };
    if (daysInfo>0 && daysInfo<=7)   return { text:`⏳ Ancora ${daysInfo} giorno${daysInfo===1?'':'i'} al tuo compleanno!`, color:'#ffb347' };
    if (daysInfo<0 && daysInfo>=-7)  return { text:`🎉 Auguri! ${Math.abs(daysInfo)} giorno${Math.abs(daysInfo)===1?'':'i'} fa!`, color:'#a8edea' };
    return null;
  })();
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        <h1 style={S.title}>✦ Specchio degli Auguri ✦</h1>
        <p style={S.subtitle}>Tantissimi auguri per il tuo compleanno...</p>
      </header>

      {/* Nuvoletta messaggio SOPRA lo specchio */}
      <AnimatePresence>
        {message && (
